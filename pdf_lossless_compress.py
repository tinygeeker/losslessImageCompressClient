...
# 尝试导入PyPDF2库，如果失败则提示用户安装
try:
    import PyPDF2
except ImportError:
    QMessageBox.critical(None, "错误", "缺少PyPDF2库，请运行 'pip install pypdf2' 安装")
    sys.exit(1)

class PDFCompressThread(QThread):
    """PDF压缩线程"""
    ...
    def run(self):
        ...
        try:
            ...
            for i in range(101):
                ...
                # 发送进度更新信号
                self.progress_update.emit(self.pdf_path, i)
            ...
            # 发送完成信号
            result = {
                'original_path': self.pdf_path,  # 添加原始路径
            ...
            # 确保发送100%进度
            self.progress_update.emit(self.pdf_path, 100)
            self.compress_finished.emit(self.pdf_path, result)
            ...
            self.progress_update.emit(self.pdf_path, 100)
            # 发送失败信号，使用空字典
            self.compress_finished.emit(self.pdf_path, {})
...
class PDFItemWidget(QWidget):
    ...
    def __init__(self, pdf_path):
        ...
        self.thumbnail_label.setStyleSheet("border: 1px solid #ddd; border-radius: 3px; background: #f0f0f0;")
        
        # 设置PDF图标文本
        self.thumbnail_label.setText("PDF")
        self.thumbnail_label.setAlignment(Qt.AlignCenter)
        self.thumbnail_label.setFont(QFont("Arial", 12, QFont.Bold))
        ...
        # 添加按钮提示
        self.download_button.setToolTip("下载PDF")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PDF无损压缩客户端")
        ...
        # 数据
        self.pdf_items = {}  # 存储PDF路径和对应的组件
        self.compress_threads = {}  # 存储压缩线程
        self.completed_pdfs = []  # 存储已完成的PDF
    ...
    def add_pdfs(self):
        ...
        file_paths, _ = QFileDialog.getOpenFileNames(
            self, "选择PDF", "", "PDF文件 (*.pdf)"
        )
        
        if file_paths:
            for file_path in file_paths:
                if file_path not in self.pdf_items:
                    # 创建PDF项组件
                    item = QListWidgetItem()
                    widget = PDFItemWidget(file_path)
                    ...
                    # 存储
                    self.pdf_items[file_path] = (item, widget)
                    
                    # 连接下载按钮
                    widget.download_button.clicked.connect(
                        lambda _, path=file_path: self.download_pdf(path)
                    )
    ...
    def start_compression(self):
        """开始压缩所有待处理PDF"""
        for pdf_path, (_, widget) in self.pdf_items.items():
            if widget.status == "等待中":
                # 创建并启动压缩线程
                thread = PDFCompressThread(pdf_path)
                thread.progress_update.connect(
                    lambda value, path=pdf_path: self.update_compress_progress(path, value)
                )
                # 捕获 pdf_path 并确保参数顺序正确
                def on_compress_finished(path, result, pdf_path=pdf_path):
                    self.handle_compress_finished(pdf_path, result)
                
                thread.compress_finished.connect(on_compress_finished)
                thread.start()
                
                # 存储线程
                self.compress_threads[pdf_path] = thread
    ...
    def update_compress_progress(self, pdf_path, value):
        """更新压缩进度"""
        if pdf_path in self.pdf_items:
            _, widget = self.pdf_items[pdf_path]
            widget.update_progress(value)
    
    def handle_compress_finished(self, pdf_path, result):
        """处理压缩完成"""
        if pdf_path in self.pdf_items:
            _, widget = self.pdf_items[pdf_path]
            ...
            if result:
                ...
                self.completed_pdfs.append(result)
    ...
    def download_pdf(self, pdf_path):
        ...
        # 查找对应的压缩结果
        result = None
        for item in self.completed_pdfs:
            # 根据原始路径查找
            if item.get('original_path') == pdf_path:
                result = item
                break
        ...
        if result:
            ...
            if save_path:
                try:
                    ...
                    QMessageBox.information(self, "成功", f"PDF已保存到: {save_path}")
                except Exception as e:
    ...
    def batch_download(self):
        """批量下载PDF"""
        if not self.completed_pdfs:
            QMessageBox.warning(self, "警告", "没有已完成的PDF可以下载")
            return
        ...
        if save_dir:
            ...
            if reply == QMessageBox.Yes:
                ...
                # 直接保存
                self.save_pdfs(save_dir)
    
    def create_zip(self, save_dir):
        """创建压缩包"""
        zip_name = f"compressed_pdfs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
        zip_path = os.path.join(save_dir, zip_name)
        
        # 创建压缩包
        try:
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                for i, result in enumerate(self.completed_pdfs):
                    # 计算进度
                    progress = (i + 1) / len(self.completed_pdfs) * 100
    ...
    def save_pdfs(self, save_dir):
        ...
        try:
            for i, result in enumerate(self.completed_pdfs):
                # 计算进度
                progress = (i + 1) / len(self.completed_pdfs) * 100
                self.statusBar().showMessage(f"正在保存PDF... {progress:.1f}%")
                QApplication.processEvents()
            ...
            self.statusBar().showMessage("就绪")
            QMessageBox.information(self, "成功", f"PDF已保存到: {save_dir}")
        except Exception as e:
            self.statusBar().showMessage("就绪")
            QMessageBox.critical(self, "错误", f"保存PDF失败: {e}")
    ...
    def show_about_dialog(self):
        ...
        label = QLabel("PDF无损压缩客户端 v1.0\n\n"
                      "一个简单易用的PDF压缩工具，支持PDF文件的无损压缩。\n\n"
                      "功能特点：\n"
                      "- 支持多PDF批量压缩\n"
                      "- 实时显示压缩进度\n"
                      "- 单张下载或批量打包下载\n"
    ...
    def show_donate_dialog(self):
        ...
        # 使用base64格式的打赏图片
        donate_base64 = "data:image/jpeg;base64,"
        # 移除data:image/jpeg;base64,前缀
        donate_base64 = donate_base64.split(',', 1)[1]
        
        # 解码base64数据
        import base64
        try:
            image_data = base64.b64decode(donate_base64)
